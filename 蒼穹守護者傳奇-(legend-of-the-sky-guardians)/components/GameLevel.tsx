...
const GameLevel: React.FC<GameLevelProps> = ({ levelId, hero, selectedTalents, onExit, onRetry }) => {
    ...
    if (deltaTime > 1000 / FRAME_RATE) {
      ...
      setGameState(prev => {
        ...
        if (newHero.isDead) {
            ...
            if (newHero.state !== 'FIGHTING' && newHero.hp < newHero.maxHp) newHero.hp += 0.2;

            if (isRangedHero && newHero.state !== 'MOVING' && newHero.state !== 'FIGHTING') {
                 const now = Date.now();
                 const attackRate = 1000 - (hero.baseStats.skillCooldown * 10); 
        ...
        if (waveStateRef.current.enemiesSpawned < waveStateRef.current.enemiesToSpawn && waveStateRef.current.spawnTimer > spawnInterval) {
            ...
            let typeIndex = 0;
            if (newWave > 10) typeIndex = Math.floor(Math.random() * 10);
            else if (newWave > 5) typeIndex = Math.floor(Math.random() * 6);
            else typeIndex = Math.floor(Math.random() * 3);
            if (newWave % 5 === 0 && Math.random() > 0.8) typeIndex = 9; 
  ...
  const callNextWave = () => {
      ...
      setGameState(prev => ({ ...prev, money: prev.money + 150, wave: prev.wave + 1 }));
      waveStateRef.current.enemiesSpawned = 0;
                ...
                >
                   {Object.values(TOWER_DEFS).map(def => (
                       <button key={def.id} onClick={() => {
                          const cost = def.t1.cost;
                          if (gameState.money >= cost) {
                              setGameState(p => ({...p, money: p.money - cost}));
                              buildTower(def.type);
                          }
